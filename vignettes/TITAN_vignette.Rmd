...
```{r}
SO <- readRDS("~/Box Sync/Alex_Data/MCF7/PEPE/RDS/PEPE_T47D.rds")
...
head(GeneDistrubition[order(GeneDistrubition[,"Topic_1"], decreasing = T),], n = 10)
...
``` {r,}
TopicWords <- TopTopicGenes(top_model, ngenes = 50)
head(TopicWords, 10)
```
...
library(tidyverse)
LDA_topics <- SO@meta.data %>% select(starts_with("Topic"))
head(LDA_topics)
```
...
Further, we find it also useful to plot two LDA topics against eachother when trying to compare how cells react to different tracriptional topics. An example being in our T47D expriment, we find many topics that are up at the late stage estrogne timepoint. Plotting 3 of the topics (11, 17 and 14)
